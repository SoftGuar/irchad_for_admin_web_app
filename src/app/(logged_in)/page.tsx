...
export default function Dashboard() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const [salesData, revenueData, devicesData, usersData, warningsData] = await Promise.all([
          fetchTotalSales(),
          fetchRevenueData(),
          fetchTotalDevices(),
          fetchTotalUsers(),
        ...
        const processedChartData = revenueData
          ...
          .map((item: { sale_period: string | number | Date; total_revenue: any; }) => ({
        ...
        setCardData(prev =>
          prev.map(card => {
            if (card.title === "Total Sales") {
              return { ...card, value: `${salesData[0]?.total_revenue ?? 0} D.A`, percentage: (salesData[0]?.total_revenue ?? 0) / 250 * 100 };
            }
            if (card.title === "Warnings") {
              return { ...card, value: totalWarnings, percentage: (totalWarnings / 10) * 100 };
            }
