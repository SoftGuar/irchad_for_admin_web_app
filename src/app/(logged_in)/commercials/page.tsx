...
const UserPage = () => {
  const [commercialAccounts, setCommercialAccounts] = useState([]);
  ...
  const fetchUsers = async () => {
    try {
      setLoading(true)
      const response = await getUsers("comemrcial"); 
      if(response.success){
        ...
        setCommercialAccounts(displayData); 
      }else {
        setError(response?.message || 'Failed to fetch commercials');
      }

    } catch (error) {
      setError('An error occurred while fetching commerials');
  ...
  return (
    <div className="flex flex-col w-full min-h-screen pb-5">
      ...
      <div className="flex -mt-10 justify-center items-start min-h-screen w-full z-0">
        <div className="w-[95%]">
          <AccountList title="Commercial" accountsData={commercialAccounts} onChange={fetchUsers}/>
        </div>
      </div>
    </div>
  );
