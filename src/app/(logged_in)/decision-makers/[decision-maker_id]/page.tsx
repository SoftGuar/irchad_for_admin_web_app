...
const UserPage = () => {
  const  decisionMaker_id  = useParams()["decision-maker_id"];
  ...
  useEffect(() => {
    const getPersonalInfos = async () => {
      try {
        const response = await getUserById("decider", String(decisionMaker_id)); 
        if(response.success){
    ...
    return (
      ...
      const onSave = async () => {
        try {
          ...
          const response = await editUser("decider", user.id, payload);
          if (response.success){
            setLoading(false);
            setIsEditing(false);
          }else{
